...
- `contract-intake` 升级至 1.0.6：把单法域团队遇到的域外合同明确记录为 `SCOPE-JURISDICTION-OUT-OF-SCOPE`，继续通用文档治理并要求转介；只有团队支持该法域却加载错误规则包时才阻断。
- 更新成员锁定至 `7c3f436852aa258259a166f3b376243587bfe915` 及对应 `v3` 内容摘要。
...
- `contract-review-lead` 升级至 1.0.18：每次成员真实回执返回后立即逐行回写覆盖矩阵、写入回执引用并重算摘要；交付前校验矩阵行状态与计数一致，避免产物已存在但矩阵仍全部 blank。
